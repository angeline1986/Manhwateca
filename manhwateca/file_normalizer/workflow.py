...
normalize_chapter_name = naming.normalize_chapter_name
detect_conflicts = conflict_rules.detect_conflicts
detect_duplicates = conflict_rules.detect_duplicates
normalize_for_duplicate_detection = (
    conflict_rules.normalize_for_duplicate_detection
)


def build_plan():
    return planner.build_plan(MANGA_ROOT, get_canonical_manga_name)


def apply_plan(plan, conflicts):
    return executor.apply_plan(plan, conflicts, dry_run=DRY_RUN)
