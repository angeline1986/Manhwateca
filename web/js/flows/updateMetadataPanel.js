...
export function renderUpdateMetadataPanel(metadata = {}) {
  ...
  const totalPendingFields = works.reduce(
    (total, work) => total + (work.pendingMetadata?.length || 0),
    0,
  );
  const description = "Selecione as obras que terão dados oficiais atualizados via MangaUpdates.";

  return `
    <section class="metadata-main-card"
             data-metadata-page="${page}"
             data-metadata-page-size="${pageSize}">
      <details class="flow-section-details metadata-stage" open>
        <summary class="flow-section-summary metadata-stage-summary">
          <div class="metadata-stage-title">
        ...
        <div class="flow-section-body metadata-stage-body">
          <header class="metadata-header">
            <p>Revise as obras prontas e selecione quais terão os metadados oficiais atualizados.</p>
          ...
          <section class="metadata-selection">
            ...
            <div class="metadata-list" data-metadata-list>
              ${works.length
                ? works.map((work, index) => metadataItem(work, index)).join("")
                : emptyState()}
            </div>

            <div class="metadata-pager-row">
              ${metadataPager(page, pages)}
            </div>
          </section>

          <footer class="metadata-action-bar">
            <div class="metadata-action-copy">
              <small>PRÓXIMA AÇÃO</small>
              <strong>Atualizar metadados</strong>
              <p>Selecione ao menos uma obra para continuar.</p>
            </div>

            <div class="metadata-actions">
              <button class="metadata-button-secondary"
                      type="button"
                      data-flow-subtab="decisoes">
                Voltar para decisões
              </button>

              <button class="metadata-button-primary"
                      type="button"
                      data-flow-run-stage
                      data-metadata-run
                      disabled>
        ...
        <div class="metadata-info-content">
          ...
          <div class="metadata-info-panel">
            ...
            <div class="metadata-fields-grid">
              ${FIELDS.map(field =>
                `<span class="metadata-field-chip">${escapeHtml(field)}</span>`
              ).join("")}
            </div>
          </div>
        </div>
...
function headingTooltip(label, text) {
  return `<span class="flow-heading-tooltip"
                tabindex="0"
                aria-label="${escapeHtml(text)}">${escapeHtml(label)}</span>`;
}
  ...
  return `
    <article class="metadata-item"
      ...
      <div class="metadata-item-header"
        ...
        <input class="metadata-item-checkbox"
               type="checkbox"
               data-metadata-choice
               data-metadata-work-id="${escapeHtml(String(work.mangaId || work.id || ""))}"
               data-metadata-fields="${count}"
               aria-label="Selecionar ${escapeHtml(title)}">

        <div class="metadata-item-content">
          <strong class="metadata-item-title">${escapeHtml(title)}</strong>
          <span class="metadata-item-meta">
            ID ${escapeHtml(String(id))} · ${selectedLabel(count, "campo pendente", "campos pendentes")}
          </span>
        </div>
...
function metadataPager(page, pages) {
  ...
  return `
    <nav class="metadata-pager"
         data-metadata-pager
         aria-label="Paginação de obras">
      <button class="flow-page-link"
              type="button"
              data-metadata-page-action="prev"
              ${page <= 1 ? "disabled" : ""}
              aria-label="Página anterior">‹</button>

      <button class="flow-page-link active"
              type="button"
              data-metadata-page-number="${page}">${page}</button>

      <button class="flow-page-link"
              type="button"
              data-metadata-page-number="${nextPage}"
              ${page >= pages ? "hidden" : ""}>${nextPage}</button>

      <button class="flow-page-link"
              type="button"
              data-metadata-page-action="next"
              ${page >= pages ? "disabled" : ""}
...
function pendingBlock(fieldKey) {
  const mapping = {
    cover: {
      label: "Capa",
      detail: "A imagem será buscada no MangaUpdates e salva localmente.",
    },
    mangaupdatesUrl: {
      label: "URL MangaUpdates",
      detail: "O link oficial será gerado com base no ID confirmado.",
    },
  };

  const info = mapping[fieldKey] || {
    label: fieldKey,
    detail: "Este campo será atualizado com os dados oficiais.",
  };
